...
def determine_complete_orbits(acq_list_dict, gunw_list, completed_list):
    ...
    for orbit in orbit_gunw_list.keys():
        ...
        if complete is True:
            for key in acq_list_hashed_dict.keys():
                if completed_hashed_dict.get(key, False) is not False:
...
def get_objects(object_type, aoi, track_number):
    ...
    #determine index
    idx_dct = {'acq-list': 'grq_*s1-gunw-acq-list', 'ifg':'grq_*_s1-gunw', 'ifg-cfg':'grq_*_s1-gunw-ifg-cfg', 'ifg-merged': 'grq_*_s1-gunw-merged', 'ifg-completed': 'grq_*_s1-gunw-aoi-track', 'ifg-merged-completed': 'grq_*_s1-gunw-merged-aoi-track'}
...
def sort_by_orbit(es_result_list):
    ...
    for result in es_result_list:
        orbit = '_'.join(result.get('_source', {}).get('metadata', {}).get('orbit', []))
